...
from qubx.cli.deploy import deploy_strategy, setup_uv_environment
...
class TestSetupUvEnvironmentFrozen:
    """Tests that `setup_uv_environment` uses `--frozen` iff the dir ships a uv.lock (issue #398)."""
    ...
    @patch("subprocess.run")
    def test_uses_frozen_when_lock_present(self, mock_subprocess, temp_dir):
        """A shipped uv.lock is authoritative: the pod must never re-resolve."""
        mock_subprocess.return_value = MagicMock(returncode=0)
        with open(os.path.join(temp_dir, "uv.lock"), "w") as f:
            f.write("# This file is automatically @generated by uv.\n")

        result = setup_uv_environment(temp_dir)

        assert result is True
        install_call_args = mock_subprocess.call_args_list[0][0][0]
        assert install_call_args == ["uv", "sync", "--frozen"]

    @patch("subprocess.run")
    def test_plain_sync_when_lock_absent(self, mock_subprocess, temp_dir):
        """No shipped lock (legacy release) falls back to a plain, re-resolving sync."""
        mock_subprocess.return_value = MagicMock(returncode=0)
        ...
        assert result is True
        install_call_args = mock_subprocess.call_args_list[0][0][0]
        assert install_call_args == ["uv", "sync"]
