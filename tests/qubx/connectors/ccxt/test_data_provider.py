...
class TestUnsubscriptionLogic:
    ...
    def test_partial_unsubscription(self, data_provider, btc_instrument):
        """Test partial unsubscription triggers resubscription."""
        eth_instrument = Instrument(
            symbol="ETHUSDT", asset_type=AssetType.CRYPTO, market_type=MarketType.SWAP,
            exchange="TEST", base="ETH", quote="USDT", settle="USDT",
            exchange_symbol="ETHUSDT", tick_size=0.01, lot_size=0.001, min_size=0.001,
        )
        
        # Mock subscription manager
        with patch.object(data_provider._subscription_manager, 'get_subscribed_instruments') as mock_get_subs:
            # First call returns both instruments, second call returns only ETH
            mock_get_subs.side_effect = [
                [btc_instrument, eth_instrument],  # Before removal
                [eth_instrument]  # After removal
            ]
            
            with patch.object(data_provider._subscription_manager, 'remove_subscription'):
                with patch.object(data_provider._data_type_handler_factory, 'get_handler', return_value=Mock()):
                    with patch.object(data_provider._subscription_orchestrator, 'execute_subscription') as mock_resub:
                        data_provider.unsubscribe("trades", [btc_instrument])
                        
                        # Should call resubscription with remaining instrument
                        mock_resub.assert_called_once()
                        call_args = mock_resub.call_args
                        remaining_instruments = call_args.kwargs['instruments']
                        assert eth_instrument in remaining_instruments
                        assert btc_instrument not in remaining_instruments
...
class TestDelegation:
    ...
    def test_get_subscriptions_delegates_to_manager(self, data_provider, btc_instrument):
        ...
        with patch.object(data_provider._subscription_manager, 'get_subscriptions', return_value=["trades"]) as mock_get_subs:
    ...
    def test_has_pending_subscription_delegates_to_manager(self, data_provider, btc_instrument):
        ...
        with patch.object(data_provider._subscription_manager, 'has_pending_subscription', return_value=False) as mock_has_pending:
