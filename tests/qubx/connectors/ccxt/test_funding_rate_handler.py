...
class TestSimplifiedFundingHandler:
    ...
    async def test_subscriber_emits_funding_rates_for_rate_subscription(
        self, mock_convert, mock_find_instrument, funding_handler, btc_instrument, rate_channel
    ):
        """Test that the subscriber emits funding rates when subscribed to funding_rate."""
        instruments = {btc_instrument}
        ...
        # Should emit funding rate (subscribed to funding_rate)
    ...
    async def test_subscriber_emits_payments_when_interval_changes(
        ...
        # Should have no data yet (no payment triggered on first funding rate)
        assert len([d for d in sent_data if d[1] == DataType.FUNDING_RATE]) == 0
        ...
        # Should only emit payment (since we subscribed to funding_payment)
        ...
        assert len(funding_rates) == 0  # No rates emitted for funding_payment subscription
