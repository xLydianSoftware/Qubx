...
def test_deque_inside_jitclass():
    ...
    @jitclass(
        [
            ("dq", DequeInt32.class_type.instance_type),
            ("counter", types.int32),
        ]
    )
    class TestClass:
...
def test_deque_indicator():
    ...
    dq.push_back_fields(timestamp=1000, value=1.5)
    dq.push_back_fields(timestamp=2000, value=2.5)
    ...
    dq.push_back_fields(timestamp=3000, value=3.5)
    dq.push_back_fields(timestamp=4000, value=4.5)  # Should overwrite first record
