...
from qubx.core.lookups import FeesLookup
...
class TestFeesLookup:
    def test_find_existing_fees(self):
        ...
        costs = fees_lookup.find("binance", "vip0_usdt")
        ...
        costs = fees_lookup.find("kraken", "k0")
    ...
    def test_find_with_direct_maker_taker_spec(self):
        ...
        costs = fees_lookup.find("any_exchange", "maker=0.05,taker=0.08")
    ...
    def test_find_nonexistent_fees(self):
        ...
        with pytest.raises(ValueError, match="No fees found for nonexistent_exchange_nonexistent_spec"):
            fees_lookup.find("nonexistent_exchange", "nonexistent_spec")

    def test_getitem_with_regex(self):
        """Test the __getitem__ method with regex pattern matching."""
        fees_lookup = FeesLookup()

        # Test finding all binance fees
        binance_fees = fees_lookup["binance.*"]
        assert len(binance_fees) > 0
        for key, values in binance_fees:
            assert key.startswith("binance")
            assert isinstance(values, tuple)
            assert len(values) == 2

        # Test finding specific tier fees
        vip0_fees = fees_lookup[".*vip0.*"]
        assert len(vip0_fees) > 0
        for key, values in vip0_fees:
            assert "vip0" in key
            assert isinstance(values, tuple)
            assert len(values) == 2
    ...
    def test_refresh_and_load(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch("qubx.core.lookups.makedirs", return_value=temp_dir):
                fees_lookup = FeesLookup(path=temp_dir)
