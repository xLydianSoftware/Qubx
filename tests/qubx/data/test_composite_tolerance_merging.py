...
class TestCompositeReaderToleranceMerging:
    ...
    def test_tolerance_deduplication_multiindex_keep_last(self):
        ...
        data1 = pd.DataFrame({"funding_rate": [0.0001, 0.0002], "source": ["reader1", "reader1"]})
        ...
        expected_timestamps = [base_time + pd.Timedelta(seconds=30), base_time + pd.Timedelta(hours=8, seconds=30)]
    ...
    def test_tolerance_deduplication_multiindex_keep_first(self):
        ...
        data2 = pd.DataFrame({"funding_rate": [0.00011, 0.00021], "source": ["reader2", "reader2"]})
        data2.index = pd.MultiIndex.from_tuples(
            [
                (base_time + pd.Timedelta(seconds=30), "BTCUSDT"),
                (base_time + pd.Timedelta(hours=8, seconds=30), "BTCUSDT"),
    ...
    def test_tolerance_deduplication_per_symbol(self):
        ...
        data1 = pd.DataFrame({"funding_rate": [0.0001, 0.0003], "source": ["reader1", "reader1"]})
        data1.index = pd.MultiIndex.from_tuples(
            [
                (base_time, "BTCUSDT"),
                (base_time, "ETHUSDT"),
        ...
        data2 = pd.DataFrame({"funding_rate": [0.00011, 0.00031], "source": ["reader2", "reader2"]})
        data2.index = pd.MultiIndex.from_tuples(
            [
                (base_time + pd.Timedelta(seconds=30), "BTCUSDT"),
    ...
    def test_strict_tolerance_no_deduplication(self):
        ...
        data1 = pd.DataFrame({"funding_rate": [0.0001], "source": ["reader1"]})
        data1.index = pd.MultiIndex.from_tuples(
            [
                (base_time, "BTCUSDT"),
            ],
        ...
        data2 = pd.DataFrame({"funding_rate": [0.00011], "source": ["reader2"]})
        data2.index = pd.MultiIndex.from_tuples(
            [
                (base_time + pd.Timedelta(seconds=30), "BTCUSDT"),
            ],
    ...
    def test_tolerance_deduplication_single_index(self):
        ...
        data1 = pd.DataFrame(
            {"value": [100, 200], "source": ["reader1", "reader1"]},
    ...
    def test_tolerance_deduplication_series(self):
        ...
        data1 = pd.Series([100, 200], index=[base_time, base_time + pd.Timedelta(hours=1)])

        data2 = pd.Series(
            [101, 201], index=[base_time + pd.Timedelta(seconds=30), base_time + pd.Timedelta(hours=1, seconds=30)]
        )
    ...
    def test_complex_timestamp_clustering(self):
        ...
        data1 = pd.DataFrame({"funding_rate": [0.001, 0.001, 0.002, 0.002, 0.003, 0.003], "source": ["r1"] * 6})
        data1.index = pd.MultiIndex.from_tuples(
            [
        ...
        data2 = pd.DataFrame({"funding_rate": [0.0011, 0.0021, 0.0031], "source": ["r2"] * 3})
        data2.index = pd.MultiIndex.from_tuples(
            [
    ...
    def test_kwargs_passed_through_with_tolerance(self):
        ...
        result = composite_reader.get_aux_data(
            "funding_payment", tolerance="30s", keep="first", exchange="BINANCE.UM", symbols=["BTCUSDT"]
        )

        # Check that additional kwargs were passed through (excluding tolerance and keep)
        mock_reader.get_aux_data.assert_called_once_with("funding_payment", exchange="BINANCE.UM", symbols=["BTCUSDT"])
