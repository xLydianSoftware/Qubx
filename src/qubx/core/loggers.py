...
class CsvFileLogsWriter(LogsWriter):
    ...
    def __init__(self, account_id: str, strategy_id: str, run_id: str, log_folder="logs") -> None:
        ...
        self._portfolio_file_path = f"{path}/{self.strategy_id}_{self.account_id}_portfolio.csv"
        self._execs_file_path = f"{path}/{self.strategy_id}_{self.account_id}_executions.csv"
        self._signals_file_path = f"{path}/{self.strategy_id}_{self.account_id}_signals.csv"

        self._hdr_pfl = not os.path.exists(self._portfolio_file_path)
        self._hdr_exe = not os.path.exists(self._execs_file_path)
        self._hdr_sig = not os.path.exists(self._signals_file_path)
    ...
    def _do_write(self, log_type, data):
        match log_type:
            ...
            case "portfolio":
                with open(self._portfolio_file_path, "w", newline="") as f:
                    w = csv.writer(f)
                    if self._hdr_pfl:
                        w.writerow(self._header(data[0]))
                        self._hdr_pfl = False
            ...
            case "executions":
                with open(self._execs_file_path, "w", newline="") as f:
                    w = csv.writer(f)
                    if self._hdr_exe:
                        w.writerow(self._header(data[0]))
                        self._hdr_exe = False
            ...
            case "signals":
                with open(self._signals_file_path, "w", newline="") as f:
                    w = csv.writer(f)
                    if self._hdr_sig:
                        w.writerow(self._header(data[0]))
                        self._hdr_sig = False
