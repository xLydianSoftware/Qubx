...
class IConnector(Protocol):
    ...
    # Sets the CONFIGURED leverage, not the venue's maximum — that is a venue-reported cap,
    # read with get_max_instrument_leverage. (Named set_max_instrument_leverage until 2026-08.)
    def set_instrument_leverage(self, instrument: Instrument, leverage: float) -> bool: ...

    # Several instruments at once. A connector that can issue them concurrently should do so:
    # the single call blocks for a venue round trip, and the caller is the ProcessorThread.
