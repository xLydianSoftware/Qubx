...
class StaleDataDetector:
    ...
    def is_instrument_stale(self, instrument: Instrument) -> bool:
        ...
        try:
            ...
            needs_reset = state.last_checked_bar_time is None or len(ohlcv) == 0
            ...
            if not needs_reset and state.last_checked_bar_time is not None:
                ...
                if (
                    newest_bar_time is None
                    or newest_bar_time < state.last_checked_bar_time
                    or
                    # Check if the data structure looks fundamentally different
                    (
                        state.last_bar_price is not None
                        and ohlcv
                        and abs(ohlcv[0].close - state.last_bar_price) > state.last_bar_price * 0.5
                    )
                ):  # 50% price change indicates reset
                    needs_reset = True
    ...
    def _perform_initial_analysis(
        self, ohlcv, state: InstrumentStaleState, timeframe_ns: int, detection_period_ns: int, current_time: dt_64
    ) -> bool:
        """
    ...
    def _process_bar_incrementally(self, bar, state: InstrumentStaleState, timeframe_ns: int) -> None:
        ...
        price_same_as_previous = state.last_bar_price is None or bar.open == state.last_bar_price
