...
class DelistingDetector:
    def __init__(self, time_provider: ITimeProvider, delisting_check_days: int = 1):
    ...
    def detect_delistings(self, instruments: list[Instrument]) -> list[Instrument]:
        ...
        for instrument in instruments:
            ...
            try:
                ...
                logger.warning(f"[DelistingDetector] Error checking delist date for {instrument}: {e}")
    ...
    def filter_delistings(self, instruments: list[Instrument]) -> list[Instrument]:
        ...
        for instrument in delisting_instruments:
            logger.info(
                f"[DelistingDetector] Filtering out {instrument.symbol} - delisting on {instrument.delist_date}"
            )
